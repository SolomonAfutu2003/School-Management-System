      ...
      <section className='flex justify-between items-center'>
        <div className='flex gap-3 items-center'>
          ...
          <div className='relative'>
            ...
            {activeFilterMenu && (
              <div className='border border-gray-400 bg-white w-64 h-[416px] absolute top-12 z-20 p-3 space-y-5'>
                <div>
                  <h3>Gender</h3>
                ...
                <div>
                  ...
                  >
                    ...
                    {TeachersData.teachers.map((teacher) => (
                      <option key={teacher.teacher_id} value={teacher.classes_assigned}>
                ...
                <div>
                  ...
                  >
                    <option value="">All Classes</option>
                    {TeachersData.teachers.map((teacher) => (
                      <option key={teacher.teacher_id} value={teacher.subjects}>
                        {teacher.subjects}
                      </option>
                    ))}
                  </select>
      ...
      <section className='border rounded-2xl border-[#EAECF0] p-5 space-y-5'>
        ...
        <div className='p-5 bg-[#F1F4F9]'>
          <ul className='grid grid-cols-16 gap-4'>
            ...
            <li className='col-span-2'>Subject(s)</li>
        ...
        {TeachersData.teachers.map((teacher) => (
          <div key={teacher.teacher_id}>
            <ul className='grid grid-cols-16 gap-4 text-[15px] p-2'>
              <li className='col-span-3'>{teacher.first_name} {teacher.last_name}</li>
              <li className='col-span-2'>{teacher.classes_assigned[0]}</li>
              <li className='col-span-2'>{teacher.subjects}</li>
              <li className='col-span-2'>{teacher.gender}</li>
              <li className='col-span-3'>{teacher.email}</li>
              <li className='col-span-2'>{teacher.phone}</li>
              <li className='col-start-16 relative'>
                <Button
                  icon={<SlOptionsVertical />}
                  onClick={() =>
                    setActiveDropdown((prev) => (prev === teacher.teacher_id ? null : teacher.teacher_id))
                  }
                />
                {activeDropdown === teacher.teacher_id && (
                  <div className='w-60 bg-white absolute right-0 z-20 shadow-lg shadow-black'>
                    <div className="flex flex-col justify-between">
                      <LinkBtn
                        to={`/admin/view-profile/${teacher.teacher_id}`}
